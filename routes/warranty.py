...
@warranty_bp.route("/filter_by_city", methods=["POST"])
@login_required
@role_required("Admin")
def filter_by_city():
    try:
        ...
        if not city or city.lower() == 'todas':
            ...
            tickets = Tickets.query.filter_by(type_of_service="2", city=city_name).order_by(
                Tickets.creation_date.desc()).all()
        ...
        for ticket in tickets:
            ...
            problems_text = ", ".join([p.name for p in ticket.problems]) if ticket.problems else ""
            
            # Obtener referencia formateada
            reference = ticket.reference
            if reference:
                reference = reference.replace('seminuevo', '').replace('Seminuevo', '').replace('SEMINUEVO', '').strip()
            
            ticket_dict = {
                'id_ticket': ticket.id_ticket,
                'reference': reference or "",
                'client_name': f"{ticket.client_info.name} {ticket.client_info.lastname}" if ticket.client_info else "",
                'state': ticket.state,
